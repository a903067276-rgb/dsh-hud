 ...
 * 也不显示错的数），没配路径就宽松找。
 */
export function parseBalanceCustom(data, path) {
...
export function createBalanceCollector(options = {}) {
  ...
  async function collect(resolveKey, call = {}) {
    ...
    const signature = [mode, targetUrl, headerName, path ?? ''].join('|')
    ...
    try {
      ...
      const value = mode === 'custom' ? parseBalanceCustom(body, path) : parseBalance(body)
