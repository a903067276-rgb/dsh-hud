        ...
        try {
          ...
          const payload = light
            ...
            : {
                ...
                git: await collectGit(shell, sandboxPolicy, sessions, sessionId, extraAll),
...
async function collectGitAt(shell, policy, cwd, sessionId, quietNonRepo) {
  try {
...
async function collectGit(shell, sandboxPolicy, sessions, sessionId, extraRepos) {
  const cwd = getSessionCwd(sessions, sessionId)
  ...
  const current = await collectGitAt(shell, policy, cwd, sessionId, subNames.length > 0 || hasExtra)
  ...
  for (const name of subNames) {
    const r = await collectGitAt(shell, policy, join(cwd, name))
    if (r !== null) pushRepo({ ...r, name, active: false }, join(cwd, name))
  }
  if (hasExtra) {
    for (const extra of extraRepos) {
      if (typeof extra !== 'string' || extra === '' || !existsSync(extra)) continue
      const r = await collectGitAt(shell, policy, extra)
      ...
      for (const sub of scanSubRepos(extra)) {
        const sr = await collectGitAt(shell, policy, join(extra, sub))
