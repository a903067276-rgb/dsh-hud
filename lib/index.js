  ...
  try {
    ...
    ctx.effect(() => webServer.register({
      ...
      handler: (req, res) => {
        ...
        let watchers = []
        try {
          const q = req.url ? decodeURIComponent(req.url.split('?')[1] || '') : ''
          const sessionId = /session=([^&]+)/.exec(q) ? /session=([^&]+)/.exec(q)[1] : ''
          const extraRepos = /repos=([^&]+)/.exec(q) ? String(/repos=([^&]+)/.exec(q)[1]).split(',').map((s) => s.trim()).filter(Boolean) : null
          const cwd = getSessionCwd(sessions, sessionId)
