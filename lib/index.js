      ...
      handler: async (req, res) => {
        try {
          ...
          if (url.pathname === '/api/dsh-hud/diff') {
            ...
            const diff = await runGit(shell, policyFor(session), cwd, 'git diff HEAD -- ' + shq(path), MAX_DIFF_BYTES)
          ...
          const payload = light
            ? { count: cwd === null ? [] : await collectGitCount(shell, policyFor(sessions.get(sessionId)), cwd, extraRepos) }
            : {
...
async function collectGit(shell, sessions, sessionId, extraRepos) {
  ...
  const policy = policyFor(sessions.get(sessionId))
