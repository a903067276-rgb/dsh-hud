    ...
    ctx.effect(() => webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          const payload = light
            ? { count: cwd === null ? [] : await collectGitCount(shell, cwd) }
            : {
...
async function collectGitCount(shell, cwd) {
  try {
    const command = "for d in . */; do [ -d \"$d/.git\" ] && { c=$(git -C \"$d\" status --porcelain 2>/dev/null | wc -l | tr -d ' '); echo \"$d:$c\"; }; done"
