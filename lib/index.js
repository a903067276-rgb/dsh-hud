    ...
    ctx.effect(() => webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          if (url.pathname === '/api/dsh-hud/events') {
            ...
            try {
              const extraRepos = url.searchParams.get('repos')
                ? String(url.searchParams.get('repos')).split(',').map((s) => s.trim()).filter(Boolean)
                : null
          ...
          const cwd = getSessionCwd(sessions, sessionId)
          // 关注仓库（localStorage 配置，请求时携带）：逗号分隔的绝对路径列表
          let extraRepos = null
          try {
            const raw = req.url && req.url.includes('repos=') ? decodeURIComponent(req.url.split('repos=')[1].split('&')[0]) : ''
            if (raw !== '') extraRepos = raw.split(',').map((s) => s.trim()).filter(Boolean)
          } catch { /* ignore */ }
          const payload = light
...
async function collectGitCount(shell, cwd, extraRepos) {
  try {
    ...
    try {
      for (const entry of readdirSync(cwd, { withFileTypes: true })) {
        if (entry.isDirectory() && !entry.name.startsWith('.') && existsSync(join(cwd, entry.name, '.git'))) dirs.push(entry.name)
      }
    } catch { /* ignore */ }
    if (Array.isArray(extraRepos)) {
      for (const extra of extraRepos) {
        if (typeof extra === 'string' && extra !== '' && existsSync(extra)) dirs.push("'" + extra.replace(/'/g, "'\\''") + "'")
      }
    }
...
async function collectGit(shell, sessions, sessionId, extraRepos) {
  ...
  const current = await collectGitAt(shell, cwd, sessionId, subNames.length > 0)
  // 多仓库聚合（2026-08-18）：当前仓库 active，子仓库/关注仓库折叠条目
  // 根目录非仓库时（issue #4）：不整体返回 null——有子仓库则正常聚合显示，active 条目省略
  const repos = []
  if (current !== null) repos.push({ ...current, name: basename(cwd), active: true })
  ...
  if (Array.isArray(extraRepos)) {
    for (const extra of extraRepos) {
      ...
      if (r !== null) repos.push({ ...r, name: basename(extra), active: false, extra: true })
      // 关注仓库内部的多仓库工作区（嵌套子仓库，缩进一级显示）
      for (const sub of scanSubRepos(extra)) {
        const sr = await collectGitAt(shell, join(extra, sub))
        if (sr !== null) repos.push({ ...sr, name: sub, active: false, extra: true, parent: basename(extra) })
      }
    }
  }
