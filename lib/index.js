...
export function apply(ctx) {
  ...
  ctx.inject(['settings'], (settingsCtx) => {
    settingsCtx.settings.register(
      NS,
      z.object({ extraRepos: z.array(z.string()).required(false), panelWidth: z.number().required(false) }),
    )
  })
    ...
    ctx.effect(() => webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          const payload = light
            ? { count: cwd === null ? [] : await collectGitCount(shell, policyFor(sandboxPolicy, sessions.get(sessionId)), cwd, extraRepos) }
            : {
                version: await collectVersion(shell),
                git: await collectGit(shell, sandboxPolicy, sessions, sessionId, extraRepos),
