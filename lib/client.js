window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 变化检测基线（2026-08-19 修复：必须模块级——组件内声明会被每次渲染重置，变化检测永远失效）
    ...
    function apply(ctx) {
      ...
      async function fetchHud(sessionId, light) {
        ...
        try {
          ...
          if (light) {
            ...
            // light 数据不覆盖面板完整数据（此前 {count} 覆盖 data 导致面板空白卡顿）
            emit({ lightCount: total, error: null });
            return;
          } else {
            // 变化自动展开（2026-08-18）：full 落地时，未提交数变化的仓库自动展开，其余收起。
            // key 必须与渲染的折叠 key 一致："repo-" + 仓库名（此前漏前缀导致永远匹配不上）
            ...
            for (const r of repos) {
              ...
              if (prev !== undefined && prev !== n && n > 0) { next["repo-" + r.name] = false; changed = true; }
              lastRepoCounts[r.name] = n;
            }
            if (changed) {
              // 只展开有变化的仓库，其他全部收起（用户需求："哪个在修改展开哪个，其他收起来"）
              for (const r of repos) {
                if (next["repo-" + r.name] === undefined) next["repo-" + r.name] = true;
              }
              setCollapsed(function (old) { return { ...old, ...next } });
            }
          }
          emit({ data, error: null, sessionId, lightCount: undefined });
        } catch (error) {
          if (snapshot.sessionId !== sessionId) return;
      ...
      slots.inject("conversation.input.left", () => slots.register(
        ...
        (props) => {
          ...
          // 角标计数：lightCount（面板关闭时轻量轮询的独立字段）优先，full 数据按多仓库聚合
          const count = snap.lightCount !== undefined ? snap.lightCount
            : (git
      ...
      slots.inject("shell.overlay", () => slots.register(
        ...
        (props) => {
          ...
          // 关注仓库配置变更（设置面板保存后）→ 立即刷新
          react.useEffect(() => {
            const onReposChanged = () => { if (typeof current === "string") fetchHud(current, snap.open ? false : true); };
