window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 当前会话 id（seat props 优先）：0.1.7 右栏 tab seat 给 props.sessionId，
    // 浮窗/≤0.1.5 给 useSessions hook，都没有时兜底共享状态（任一 seat 拿到过就会写进去）。
    const currentSessionOf = (props, snap) => {
      const direct = props && typeof props.sessionId === "string" && props.sessionId !== "" ? props.sessionId : undefined;
      const hook = props && typeof props.useSessions === "function" ? props.useSessions((s) => s && s.current) : undefined;
      const stored = snap && typeof snap.sessionId === "string" && snap.sessionId !== "" ? snap.sessionId : undefined;
      if (direct !== undefined) return direct;
      if (typeof hook === "string" && hook !== "") return hook;
      return stored;
    };
    const retriedSessions = new Map(); // 0.1.7：git:null（会话尚未注册）时的一次性补取标记
    const WIDTH_MIN = 200;
    const WIDTH_MAX = 480;
    ...
    // 仓库条目的展开状态（2026-09-26 用户定）：按"有没有改动"自动决定——干净的收成一行，
    // 有改动的展开到文件层；用户手动点过就以用户的选择为准，并记住（localStorage，刷新/重开面板都在）。
    const REPO_OPEN_KEY = "dsh-hud-repo-open";
    const readRepoOpen = () => {
      try {
        const raw = window.localStorage.getItem(REPO_OPEN_KEY);
        if (raw) {
    ...
    const writeRepoOpen = (m) => {
      try { window.localStorage.setItem(REPO_OPEN_KEY, JSON.stringify(m)); } catch (error) { /* ignore */ }
    };
    ...
    function HudSettingsCard(props) {
      ...
      // ── 可调配置（2026-09-26 加）：提交条数 / 余额三档 / 余额预设与自定义字段 ──
      // 值存在 profile 条目 config 里（volatile → 保存即时生效）；这里只做读、改、存。
      const [cfg, setCfg] = react.useState(null);
      const [cfgSaved, setCfgSaved] = react.useState(false);
      const [cfgError, setCfgError] = react.useState("");
      react.useEffect(() => {
        let alive = true;
        fetch("/api/dsh-hud/config", { credentials: "same-origin" })
          .then((r) => r.json())
          .then((d) => { if (alive && d && d.ok) setCfg(d.config || {}); })
          .catch(() => { /* 读不到就留默认，保存仍可用 */ });
        return () => { alive = false; };
      }, []);
      const cfgField = (key, v) => setCfg(Object.assign({}, cfg || {}, { [key]: v }));
      const saveCfg = async () => {
        setCfgError("");
        try {
          const r = await fetch("/api/dsh-hud/config", {
            method: "POST",
          ...
          if (!r.ok || !d || !d.ok) { setCfgError((d && d.error) || ("HTTP " + r.status)); return; }
          setCfgSaved(true);
          if (savedTimer.current !== null) clearTimeout(savedTimer.current);
          savedTimer.current = setTimeout(() => { setCfgSaved(false); savedTimer.current = null; }, 2000);
        } catch (e) { setCfgError(String(e && e.message ? e.message : e)); }
      };
      const save = () => {
      ...
      return react.createElement("div", null,
        ...
        field(
          "提交记录条数",
          "每个仓库显示多少条提交（1~50，默认 10）。提交小节默认收起，点开才展开",
          reactElement("input", {
            className: "dsh-hud-input",
            type: "number", min: 1, max: 50,
            value: cfg ? String(cfg.commitCount === undefined || cfg.commitCount === null ? 10 : cfg.commitCount) : "10",
            onChange: (e) => cfgField("commitCount", Number(e.target.value)),
          })
        ),

        field(
          "余额显示",
          "关＝不请求也不显示；官方＝DeepSeek 官方接口（只认官方 key）；自定义＝你自己的接口（下面填）",
          reactElement("select", {
            className: "dsh-hud-input",
            value: (cfg && cfg.balanceMode) || "official",
            onChange: (e) => cfgField("balanceMode", e.target.value),
          },
            reactElement("option", { value: "off" }, "关（不显示余额）"),
            reactElement("option", { value: "official" }, "官方（DeepSeek）"),
            reactElement("option", { value: "custom" }, "自定义接口")
          )
        ),

        field(
          "余额预设",
          "选一个服务就自动带好地址 / 请求头 / 字段路径 / 凭据变量；下面自己填的字段优先于预设。one-api 系的地址每家不同，仍需自己填",
          reactElement("select", {
            className: "dsh-hud-input",
            value: (cfg && cfg.balancePreset) || "",
            onChange: (e) => cfgField("balancePreset", e.target.value),
          },
            reactElement("option", { value: "" }, "（不用预设）"),
            reactElement("option", { value: "kimi" }, "Kimi / Moonshot"),
            reactElement("option", { value: "siliconflow" }, "SiliconFlow"),
            reactElement("option", { value: "openrouter" }, "OpenRouter（两字段相减）"),
            reactElement("option", { value: "zai" }, "z.ai"),
            reactElement("option", { value: "one-api" }, "one-api / 中转站（要填地址）"),
            reactElement("option", { value: "deepseek" }, "DeepSeek 官方")
          )
        ),

        ((cfg && cfg.balanceMode) === "custom" || (cfg && cfg.balancePreset && cfg.balancePreset !== "deepseek"))
          ? reactElement("div", null,
              field("接口地址", "完整 URL（必须 https，本机地址除外）；此地址会收到你的 API key",
                reactElement("input", { className: "dsh-hud-input", placeholder: "https://my-gateway.example.com/api/user/self",
                  value: (cfg && cfg.balanceUrl) || "", onChange: (e) => cfgField("balanceUrl", e.target.value) })),
              field("凭据环境变量名", "留空＝DEEPSEEK_API_KEY；one-api 系要填**系统访问令牌**所在的环境变量名",
                reactElement("input", { className: "dsh-hud-input", placeholder: "MY_GATEWAY_KEY",
                  value: (cfg && cfg.balanceTokenEnv) || "", onChange: (e) => cfgField("balanceTokenEnv", e.target.value) })),
              field("请求头名", "默认 authorization（值形如 Bearer <key>）；填 x-api-key 之类则原样放",
                reactElement("input", { className: "dsh-hud-input", placeholder: "authorization",
                  value: (cfg && cfg.balanceHeader) || "", onChange: (e) => cfgField("balanceHeader", e.target.value) })),
              field("余额字段路径", "如 data.balance、balance_infos[0].total_balance；留空则宽松找常见字段",
                reactElement("input", { className: "dsh-hud-input", placeholder: "data.balance",
                  value: (cfg && cfg.balancePath) || "", onChange: (e) => cfgField("balancePath", e.target.value) })),
              field("单位换算", "one-api 系 data.quota 是原始额度单位，如每元 500000 → 填 500000；不换算保持 1",
                reactElement("input", { className: "dsh-hud-input", type: "number", min: 0,
                  value: cfg && cfg.balanceScale !== undefined && cfg.balanceScale !== null ? String(cfg.balanceScale) : "1",
                  onChange: (e) => cfgField("balanceScale", Number(e.target.value)) })),
              field("需减去的字段", "OpenRouter 之类要「总额 − 已用」时填已用字段路径，如 data.total_usage",
                reactElement("input", { className: "dsh-hud-input", placeholder: "（不用就留空）",
                  value: (cfg && cfg.balanceSubtractPath) || "", onChange: (e) => cfgField("balanceSubtractPath", e.target.value) })),
              field("额外请求头", "需要第二个头时写 “Name: value”，如 Dmx-Api-User: 12345",
                reactElement("input", { className: "dsh-hud-input", placeholder: "（不用就留空）",
                  value: (cfg && cfg.balanceExtraHeader) || "", onChange: (e) => cfgField("balanceExtraHeader", e.target.value) }))
            )
          : null,

        field(
          "保存上面的设置",
          "写进当前 profile 的插件配置（volatile 字段即时生效，不用重启）",
          null,
          [ reactElement("button", {
              key: "save-cfg", type: "button",
              className: "dsh-hud-btn" + (cfgSaved ? " dsh-hud-btn-saved" : ""),
              onClick: saveCfg,
            }, cfgSaved ? "✓ 已保存" : "保存配置"),
            cfgSaved ? reactElement("span", { className: "dsh-hud-saved-note" }, "已即时生效") : null,
            cfgError ? reactElement("span", { className: "dsh-hud-saved-note" }, "保存失败：" + cfgError) : null ]
        )
      );
    }
    ...
    function apply(ctx) {
      ...
      async function fetchHud(sessionId, light) {
        ...
        try {
          ...
          // 0.1.7 修（2026-09-24 影子实测三）：共享状态里的会话在 0.1.7 上可能始终为空
          // （seat 不再下发 useSessions → 输入框按钮也没得写），此时按"请求与守卫同源"会把
          // **每一个**响应都丢掉（实测：resp gitNull=false mcp=2 仍被 DROPPED store=null）。
          // 守卫只在"共享状态里确实有另一个会话"时才丢弃（仍防串台）。
          if (typeof snapshot.sessionId === "string" && snapshot.sessionId !== "" && snapshot.sessionId !== sessionId) return;
          ...
          if (light) {
            ...
            // 0.1.7 补（2026-09-24 影子实测）：新建会话的首个 full 响应可能早于宿主注册该会话
            // （host 侧 sessions.get(id) 还是空 → cwd 未知 → git:null），面板就会一直停在
            // "不是 Git 仓库"。对同一会话只补一次延迟重取（1.5s），拿到真数据即止。
            if (data && data.git === null && retriedSessions.get(sessionId) !== true) {
              retriedSessions.set(sessionId, true);
              setTimeout(() => { fetchHud(sessionId, false); }, 1500);
            }
          }
        } catch (error) {
          // 0.1.7 修（2026-09-24 影子实测三）：共享状态里的会话在 0.1.7 上可能始终为空
      ...
      slots.inject("conversation.input.left", () => slots.register(
        ...
        (props) => {
          ...
          // 0.1.7 修（2026-09-24）：浮窗/右栏两个 seat 都不再下发 useSessions（tab seat 只给
          // hookContext，浮窗也不给），props 拿不到会话 → fetchHud 直接 return，host 数据
          // （git/MCP/技能/余额）整块空成"不是 Git 仓库"。回落用按钮组件同步进共享状态的
          // sessionId（同一个会话，按钮在输入框 seat 里始终能拿到）。
          // 0.1.7 修（2026-09-24 影子实测二）：**优先用共享状态里的会话**——fetchHud 的响应
          // 会被 `snapshot.sessionId !== sessionId` 守卫丢弃（防串台），若请求用的是 props
          // 里的另一个会话（浮窗/右栏 seat 给的 current 与输入框按钮可能不同），响应会被
          // 静默丢掉 → 面板永远 data=null，看起来就是"不是 Git 仓库"且 MCP/技能全 0。
          // 共享状态由输入框按钮（唯一能拿到会话的 seat）写入，请求与守卫同源即一致。
          const current = currentSessionOf(props, snap);
      ...
      function HudPanel(props) {
        const variant = props.variant === "tab" ? "tab" : "float";
          ...
          // 仓库条目展开状态（2026-09-26）：默认由"有没有改动"决定，用户点过则以用户为准并持久化
          const [repoOpen, setRepoOpen] = react.useState(readRepoOpen);
          // 各小节收拢状态
          ...
          // 会话来源（2026-09-24 定稿，三次踩坑后）：0.1.7 的右栏 tab seat 下发的是
          // **props.sessionId**（官方 ui-sidebar-files 同款用法），浮窗/旧宿主才给
          // useSessions；都没有时兜底共享状态。缺了这条，右栏 tab 形态下面板一次请求都不发
          // ——表现就是"Git/MCP/技能/余额全空、只剩客户端投影"。
          const current = currentSessionOf(props, snap);
          ...
          const section = (key, title, count, content, defaultCollapsed) => {
            const isCollapsed = collapsed[key] === undefined ? defaultCollapsed === true : collapsed[key] === true;
          ...
          return react.createElement("div", { style: variant === "tab" ? tabStyle : floatStyle },
            ...
            react.createElement("div", {
              ...
              version !== null
                ...
                : null,
              // 余额（自动拉取；数据未到/失败/无凭据/模式关显示 --）
              // 注意：snap.data 未加载时 balance 是 undefined，必须用 != null 宽松判断
              // 2026-09-26：自定义接口（balanceMode: custom）只回一个总额，标题就别硬说"官方余额/赠金"
              react.createElement("span", {
                title: balance != null
                  ? (balance.source === "custom"
                      ? "余额（自定义接口）"
                      : "官方余额 · 含赠金 ¥" + balance.granted + " · 充值 ¥" + balance.toppedUp)
                  : "余额不可用（模式关闭 / 无凭据 / 接口失败——可在插件配置里设 balanceMode）",
                ...
                : (function () {
                    ...
                    const repoBody = (repo, clickable) => {
                      ...
                      return react.createElement("div", null,
                        ...
                        cmts.length > 0
                          ? section("commits-" + repo.name, "提交", cmts.length,
                              react.createElement("ul", { style: { listStyle: "none", margin: 0, padding: 0 } },
                              ...
                              // 提交记录默认收起（2026-09-26）：内容取全（条数由 commitCount 配，默认 10），
                              // 但别一展开仓库就糊一屏——想看再点开
                              true
                            )
                          : null
                    ...
                    const repoBlock = (repo, indent) => {
                      ...
                      // 2026-09-26（用户定）：干净 → 默认折叠成一行；有改动 → 默认展开到文件层。
                      // 用户手动点过之后（repoOpen 里有记录）以用户为准，并写进 localStorage 记住。
                      const dirty = (repo.files || []).length > 0;
                      const override = repoOpen[rkey];
                      const isCollapsed = typeof override === "boolean" ? !override : !dirty;
                      const toggle = () => {
                        const next = { ...repoOpen, [rkey]: isCollapsed };
                        setRepoOpen(next);
                        writeRepoOpen(next);
                      };
                      return react.createElement("div", { key: rkey },
                        react.createElement("div", {
                          ...
                          // 折叠时也别把"未推送/落后"藏掉（干净仓库头一行就能看全）
                          isCollapsed && ((repo.ahead || 0) > 0 || (repo.behind || 0) > 0)
                            ? react.createElement("span", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: "11px", flexShrink: 0 } },
                                "↑" + (repo.ahead || 0) + " ↓" + (repo.behind || 0))
                            : null,
                          react.createElement("span", {
    ...
    if (slots !== undefined) {
      ...
      const ROW_CONFIG_KEY = "dsh-hud#dsh-hud";
      slots.inject("plugins.row.config", () => slots.register(
        { name: "plugins.row.config", key: ROW_CONFIG_KEY, order: 30, label: "HUD" },
      ...
      slots.inject("plugins.bundle.config", () => slots.register(
        { name: "plugins.bundle.config", key: "dsh-hud", order: 30, label: "HUD" },
        () => react.createElement(HudSettingsCard, { inCard: true }),
      ));
