window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // light 轮询上次未提交总数（变化检测：变了立即补 full；模块级防渲染重置）
      ...
      async function fetchHud(sessionId, light) {
        ...
        try {
          ...
          if (light) {
            ...
            // 状态驱动展开（2026-08-19，用户需求"哪个在修改展开哪个，其他收起来"）：
            // 每次 full 落地：有未提交文件的子仓库强制展开，干净的收起；active 仓库保持原状。
            // （此前"变化检测"依赖基线对比，首次 full 永远不触发，改状态驱动后不依赖任何基线）
          ...
          if (snapshot.sessionId !== sessionId) return;
          // 失败保留旧数据（2026-08-19 修：清 data 导致面板空白且可能不恢复；
          // 数据过期总比空白好，面板底部会显示错误提示）
