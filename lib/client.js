window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 官方右栏能力探测结果（apply 时赋值；设置卡片据此提示，不影响渲染）
    ...
    function apply(ctx) {
      ...
      // 形态裁决（2026-09-10）：tab 形态需要官方右栏两件套（0.1.5+）；auto 下缺任一 → 回落浮窗
      const sidebarRightTabs = ctx.get("sidebarRightTabs");
      const sidebarRight = ctx.get("sidebarRight");
      const panelMode = readPanelMode();
      const useTab = panelMode !== "off"
        && (panelMode === "tab" || (panelMode === "auto" && sidebarRightTabs !== undefined && sidebarRight !== undefined));
      tabCapable = sidebarRightTabs !== undefined && sidebarRight !== undefined;
      ...
      slots.inject("conversation.input.left", () => slots.register(
        ...
        (props) => {
          ...
          const toggle = () => {
            ...
            if (useTab && sidebarRight !== undefined) {
              try {
                const active = typeof sidebarRight.active === "function" ? sidebarRight.active() : undefined;
                const expanded = typeof sidebarRight.isExpanded === "function" ? sidebarRight.isExpanded() : false;
                const showing = expanded && active !== undefined && active !== null && active.kind === "dsh-hud";
                if (showing && typeof sidebarRight.toggleExpanded === "function") {
                  sidebarRight.toggleExpanded();
                  emit({ tabOpen: false });
                } else {
                  sidebarRight.openTab("dsh-hud");
                  emit({ tabOpen: true });
                }
              } catch (error) { /* ignore */ }
              return;
          ...
          return react.createElement("button", {
            type: "button",
            className: "dsh-hud-toggle-btn" + ((useTab ? snap.tabOpen : snap.open) ? " is-open" : ""),
            onClick: toggle,
            title: useTab
      ...
      // ── 形态注册（2026-09-10）：tab = 官方右栏标签页（0.1.5+）；float = 原浮窗（旧宿主回退）──
      if (useTab && sidebarRightTabs !== undefined) {
